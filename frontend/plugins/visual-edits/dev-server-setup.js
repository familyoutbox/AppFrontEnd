...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all AppGenie.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*AppGenie\.sh$/)) {
        return true;
      }

      // Allow all AppGenieagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*AppGenieagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@AppGenie.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@AppGenie.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
